...
*)
  ...
  | alseq r x y => before r (store_sys x)
                         (store_sys y)

                         
  | abseq r s x y =>  before r (store_sys x)
                         (store_sys y)
...
Lemma lseq_assoc:
  forall t1 t2 t3 i,
    same_rel
      (store_sys (snd (anno (lseq t1 (lseq t2 t3)) i)))
      (store_sys (snd (anno (lseq (lseq t1 t2) t3) i))).
Proof.
  intros; simpl.
