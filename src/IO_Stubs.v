...
Definition do_remote (t:Term) (pTo:Plc) (e:EvC) (ac: AM_Config) : ResultT RawEv CallBackErrors := 
  let remote_uuid_res : ResultT UUID CallBackErrors := ac.(plcCb) pTo in
    match remote_uuid_res with 
    | resultC uuid => doRemote_uuid t uuid (get_bits e)
    | errC e => errC e 
    end.
